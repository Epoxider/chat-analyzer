...
from tensorflow.keras.layers import StringLookup, Embedding, Input, Dense, Flatten, Dropout, Concatenate
...
y_pred_probs = model.predict([X_text, X_time, X_user, X_channel, X_game, X_subscriber, X_mod])
...
with open('./Eval_Reports/new_model_eval_report.txt', 'a+') as f:
    f.truncate(0)
    f.write(report)